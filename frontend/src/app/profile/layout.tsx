...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <MainContainer className="flex flex-col overflow-auto md:flex-row ">
      <section className=" sticky  top-0 flex h-14 w-full items-center rounded-lg px-1 py-2 font-poorStory dark:bg-[#18181b] md:mr-4  md:h-[600px] md:w-48 md:flex-col md:gap-2 md:px-4 md:py-2">
        <ProfileLinkButton
          text="프로필"
          href={`/profile/${userId || ""}`}
        ...
        {!userId && (
          <>
            <Divider className="mx-1 h-full w-1 md:h-[1px] md:w-full" />
            <Link
              href={"/profile/like"}
